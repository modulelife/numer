	...
	namespace qm {
		...
		public:
			HusimiQCalculator(unsigned Max_energy_level_, unsigned Resolution_)
				:max_n_(Max_energy_level_ + 1), resol_(Resolution_), fcoef_(max_n_), alphas_(resol_, resol_)
			{
				...
				alphas_.refill_par(cplx_sampler);
			}

			template<class Array>
			mat<double> calculate(Array E_rep_ket_, size_t N_) {
				...
				const auto calcu_proj = [&](const args__& arg) -> void {
					const Complex a = alphas_[arg.i__][arg.j__].conj();
